...
class BrysonLQRController(Node):
    def __init__(self):
        ...
        self.Q_orig = self.Q.copy()  # Save original Q
        self.R_orig = self.R.copy()  # Save original R
        ...
        self.q_log = []   # Q diagonal values per time step
        self.r_log = []   # R scalar value per time step
        ...
        # Timers: control loop at 100 Hz; update multipliers every 5 seconds
    ...
    def compute_bryson_weights(self):
        ...
        self.get_logger().info(f"Bryson Weights: Q = diag([{q0:.4f}, {q1:.4f}, {q2:.4f}, {q3:.4f}]), R = {r_val:.6f}")

    def compute_lqr_gain(self):
        # Solve the continuous-time algebraic Riccati equation and compute K
        try:
    ...
    def update_q_r(self): #Encountered some weird errors here with the parameters. Turned out it was quite straight forward. GPT helped me with that
        ...
        self.get_logger().info(f"Update at t={current_time:.2f}s: q_multiplier={self.q_multiplier:.2f}, r_multiplier={self.r_multiplier:.2f}")

    def joint_state_callback(self, msg):
        # Update the state x from joint state messages
        try:
    ...
    def control_loop(self):
        # Compute u = -Kx and publish the control command; log data
        if not self.state_initialized:
            return
        u = -self.K @ self.x
        force_cmd = float(u[0])
    ...
    def save_plots_and_csv(self):
        # Save five plots and a CSV file with logged data
        if not self.t_log:
